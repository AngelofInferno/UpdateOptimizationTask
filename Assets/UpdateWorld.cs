...
public class UpdateWorld : MonoBehaviour
{
    ...
    void SimulateWorld2()
    {
        for (int y = 0; y < texture.height; y++)
        {
            for (int x = 0; x < texture.width; x++)
            {
                var count = CountNeighbourColor(x, y, Color.black);
                ...
                else if (count > 0 && pixel == Color.white)
                {
                    texture.SetPixel(x, y, Color.green);
                }
                //if near a black cell but the cell is already yellow
                //Mining Property
                if (count > 0 && pixel == Color.yellow)
                {
    ...
    void SimulateWorld(int x, int y)
    {
        /*for (int y = 0; y < texture.height; y++)
        {
            for (int x = 0; x < texture.width; x++)
        ...
        //set to green
        //Residential Property
        else if (count > 0 && texture.GetPixel(x,y) == Color.white)
        {
            texture.SetPixel(x, y, Color.green);
        }
        //if near a black cell but the cell is already yellow
        //Mining Property
        if (count > 0 && texture.GetPixel(x, y) == Color.yellow)
        {
            texture.SetPixel(x, y, Color.magenta);
        }
    ...
    void Update()
    {
        ...
        {
            RaycastHit ray;
            if (Physics.Raycast(_mainCamera.ScreenPointToRay(Input.mousePosition), out ray))
            {
